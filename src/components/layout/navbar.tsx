...
import { Menu, X, ChevronDown, Briefcase, Users, MessageSquare, Info, HomeIcon, Rocket } from "lucide-react";
  ...
  const renderNavLinks = (isMobile = false) =>
    navLinks.map((link) => {
      ...
      return link.subLinks ? (
        <DropdownMenu key={link.label} open={exploreMenuOpen && !isMobile} onOpenChange={isMobile ? undefined : setExploreMenuOpen}>
          <DropdownMenuTrigger
            asChild
            onMouseEnter={isMobile ? undefined : () => setExploreMenuOpen(true)}
            onMouseLeave={isMobile ? undefined : () => setExploreMenuOpen(false)}
          >
            <Button
              variant="ghost" 
              className={cn(
            ...
            {link.subLinks.map((subLink) => (
              ...
              >
                ...
                >
                  ...
                  <Badge variant="outline" className="ml-2 text-xs px-1.5 py-0.5 border-primary/50 text-primary/80 group-hover/item:border-accent-foreground/50 group-hover/item:text-accent-foreground group-hover/item:bg-accent">
                    Upcoming
                  </Badge>
                </Link>
              </DropdownMenuItem>
        ...
        <Button
          ...
          asChild
          key={link.label}
          className={isMobile ? "w-full justify-start text-lg py-3" : "px-3 py-2"}
        >
          <Link
            href={link.href!} 
            onClick={() => {
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          <Button onClick={handleJoinWaitingList} variant="default" className="bg-accent hover:bg-accent/90 text-accent-foreground hidden sm:inline-flex transition-colors duration-200">
            Join Waiting List
          </Button>
          <ThemeToggle />
            ...
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              ...
              <SheetContent side="right" className="w-full max-w-xs bg-background p-6">
                <div className="flex flex-col space-y-5">
                  ...
                  <Button onClick={() => { handleJoinWaitingList(); setIsMobileMenuOpen(false); }} variant="default" className="w-full bg-accent hover:bg-accent/90 text-accent-foreground mt-4 transition-colors duration-200">
                    Join Waiting List
                  </Button>
                </div>
              </SheetContent>
